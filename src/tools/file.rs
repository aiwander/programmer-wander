...
pub async fn read_file(args: Value) -> Result<Value> {
    ...
    if let Some(pattern) = search {
        ...
        if matches.is_empty() {
            return Ok(json!(format!("[NO MATCHES] '{}' not found in {} lines", pattern, total_lines)));
        } else {
            return Ok(json!(format!("[{} matches in {} lines]\n{}", matches.len(), total_lines, matches.join("\n"))));
        }
    }
...
pub async fn edit_block(args: Value) -> Result<Value> {
    ...
    let expected = args.get("expected_replacements").and_then(|v| v.as_i64()).unwrap_or(1);
