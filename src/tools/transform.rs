...
pub async fn hash_file(args: Value) -> Result<Value> {
    let path = args["path"].as_str().unwrap_or("");
    let algorithm = args["algorithm"].as_str().unwrap_or("sha256").to_uppercase();
...
pub async fn scaffold(args: Value) -> Result<Value> {
    ...
    let files_created: Vec<String> = match template {
        ...
        _ => return Ok(json!({"error": format!("Unknown template: {}. Use: rust-mcp, python-mcp, nextjs, fastapi, expo", template)})),
    };
...
"#, name);
    ...
    write_scaffold(&base.join("app/page.tsx"), "export default function Home() {\n  return <main><h1>Hello World</h1></main>\n}\n", &mut files);
...
"#, name);
    ...
    write_scaffold(&base.join("requirements.txt"), "fastapi>=0.100.0\nuvicorn>=0.23.0\n", &mut files);
...
"#, name);
    ...
    write_scaffold(&base.join("app.json"), &format!(r#"{{"expo": {{"name": "{}"}}}}"#, name), &mut files);
                ...
                if path.is_file() {
                    ...
                    for entry in WalkDir::new(path).into_iter().filter_map(|e| e.ok()) {
                        ...
                        if entry_path.is_file() {
                            let rel_path = entry_path.strip_prefix(path.parent().unwrap_or(path))
...
pub async fn extract(args: Value) -> Result<Value> {
    ...
    let destination = args["destination"].as_str()
        ...
        .unwrap_or_else(|| {
            ...
            p.parent().unwrap_or(Path::new(".")).to_string_lossy().to_string()
        });
    ...
    if ext == "zip" || archive_path.ends_with(".zip") {
        ...
        let cmd = format!(r#"tar {} "{}" -C "{}""#, extract_flag, archive_path, destination);
...
pub async fn sync_directories(args: Value) -> Result<Value> {
    ...
    let dry_run = args["dry_run"].as_bool().unwrap_or(true);
    let exclude = args["exclude"].as_array()
...
pub async fn diff_files(args: Value) -> Result<Value> {
    let path1 = args["path1"].as_str().or(args["file_a"].as_str()).unwrap_or("");
    let path2 = args["path2"].as_str().or(args["file_b"].as_str()).unwrap_or("");
