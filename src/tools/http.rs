...
pub async fn download(args: Value) -> Result<Value> {
    ...
    let elapsed = start.elapsed().as_millis() as u64;
    let final_size = fs::metadata(destination).await.map(|m| m.len()).unwrap_or(0);
...
pub async fn scrape(args: Value) -> Result<Value> {
    ...
    while i < chars.len() {
        if !in_tag && chars[i] == '<' {
            ...
            if remaining.starts_with("</script") { in_script = false; }
            if remaining.starts_with("</style") { in_style = false; }
