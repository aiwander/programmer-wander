...
static SERVERS: Lazy<Mutex<HashMap<String, WebhookServer>>> = Lazy::new(|| Mutex::new(HashMap::new()));
                    ...
                    tokio::spawn(async move {
                        ...
                        if buf_reader.read_line(&mut request_line).await.is_ok() {
                            ...
                            if parts.len() >= 2 {
                                ...
                                let action = routes.get(path).cloned().unwrap_or_else(|| "unknown".to_string());
...
pub async fn list_webhook_servers() -> Result<Value> {
    ...
    let list: Vec<Value> = servers.iter().map(|(id, s)| {
        json!({
            "server_id": id,
            "port": s.port,
